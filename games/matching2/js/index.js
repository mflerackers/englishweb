...
let consonants = {};
let vowels = {}
let letterOnGrid = false;
let letterSprites = [];

function randomColor(pos) {
    //let tries = 100;
    //while (tries--) {
        if (pos === undefined && !letterOnGrid) {
            let keys = Object.keys(consonants);
            let key = keys[Math.floor(Math.random()* Math.floor(keys.length))];
            let letter = consonants[key];
            console.log(consonants, keys, key, letter);
            letterOnGrid = true;
            return letter;
        }
...
class Pop extends Idle {
    constructor(list, trajectories, finished) {
        ...
        list.forEach((pos, i) => { this.grid[pos] = (trajectories && trajectories[i]) || true; });
...
class Drop extends Idle {
    ...
    update(dt) {
        ...
        setState(new Idle());
        // If there are holes, drop
        if (grid.some(color => color == 0)) {
            drop();
        }
        // Check the last row for drop items
        else if (grid.slice(rows*(columns-1)).some(color => color > phonemes.length)) {
            // Collect drop items from the last row and pop them
            let list = [];
            grid.slice(rows*(columns-1)).forEach((color, p) => {
                if (color > phonemes.length) {
                    list.push(rows*(columns-1) + p);
                }
            });
            // Update wether there are still consonants on the board
            // TODO: actually check for consonants
            letterOnGrid = grid.slice(0, rows*(columns-1)).some(color => color > phonemes.length);
            let trajectories = [];
            let parts = [];
            list.forEach(pos=>{
                let color = getColor(pos);
                let [grapheme, phoneme] = getGraphemePhonemeFromColor(color); // Some function
                let position = positions[`${grapheme}-${phoneme}`];
                let index = 0;
                if (position) {
                    ...
                    list.map(pos => {
                        ...
                        trajectories[index++] = calcTrajectory(0, 0, x2-dx, y2-dy);
                    });
                }
                else {
                    index++;
                }
                console.log(color, grapheme, phoneme, position);
            });
            let finished = function() {
                parts.forEach(part=>part[4] = 1);
                if (getApp().isCurrentWordFinished()) {
                    app.nextWord();
                }
            }
            console.log(list, trajectories);
            setState(new Pop(list, trajectories, finished));
            //pop(list, trajectories);
        }
...
function pop(i, j) {
    ...
    if (typeof i == 'number') {
        ...
        // If there are more than 2 connected balls
        if (list && list.length > 1) {
            sounds[color-1].play();
            let phoneme = phonemes[color-1];
            let trajectories;
            let finished;
            // Make all balls fly to the position where the user tapped
            let [x2, y2] = [margin+i*size, margin+j*size];
            ...
            // When the trajectories are finished, reveal a new ball with a phoneme if there were more than 3 balls
            if (list.length > 3) {
                finished = function() {
                    let graphemes = vowels[phoneme];
                    let keys = Object.keys(graphemes);
                    let color = graphemes[keys[Math.floor(Math.random()* Math.floor(keys.length))]];
                    console.log(graphemes, color);
                    setColor(color, i, j);
                }
            }
            setState(new Pop(list, trajectories, finished));
...
function drop() {
    ...
    // This list keeps for each column the row index of the first empty element
    let list = new Array();
    for (let i = 0; i < columns; i++) {
        for (let j = rows-1; j >= 0; j--) {
            ...
            // If this is not the top row, use the ball color of the row above
            if (j > 0) {
                setColor(getColor(i, j-1), i, j);
            }
            // If it is the top row, create a new random ball
            else {
...
function getGraphemePhonemeFromColor(color) {
    for ([grapheme, index] of Object.entries(consonants)) {
        if (index == color) {
            return [grapheme, "none"];
        }
    }
    for ([phoneme, graphemeIndex] of Object.entries(vowels)) {
        for ([grapheme, index] of Object.entries(graphemeIndex)) {
            if (index == color) {
...
class GameApp extends App {
    ...
    constructor(div) {
        ...
        let letterIndex = phonemes.length+1;
        this.words.forEach(word => {
            word.forEach(([grapheme, phoneme]) => {
                if (phoneme === "none") {
                    if (!consonants[grapheme]) {
                        consonants[grapheme] = letterIndex++;
                        letterSprites.push([grapheme, phoneme]);
                    }
                }
                else if (phoneme !== "silent") {
                    let graphemes = vowels[phoneme];
                    if (graphemes) {
                        if (!graphemes[grapheme]) {
                            graphemes[grapheme] = letterIndex++;
                            letterSprites.push([grapheme, phoneme]);
                        }
                    }
                    else {
                        vowels[phoneme] = {[grapheme]:letterIndex++};
                        letterSprites.push([grapheme, phoneme]);
                    }
                }
            });
    ...
    fillPositions() {
        this.words[this.currentWord].forEach(part => {
            ...
            let [grapheme, phoneme, px, py, level] = part;
            positions[`${grapheme}-${phoneme}`] = [px, py, part];
        });
    }
    ...
    createImageFromImageData(imagedata) {
        ...
        canvas.width = imagedata.width + spriteSize * letterSprites.length;
        canvas.height = imagedata.height;
        ctx.putImageData(imagedata, 0, 0);

        let i = phonemes.length + 1;
        let x = (i + 0.5) * spriteSize;
        let y = spriteSize*0.5;

        ctx.strokeStyle = "black";
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.font = `${spriteSize}px sans-serif`;
        letterSprites.forEach(([grapheme, color]) => {
            if (grapheme.length > 1) {
                ctx.font = `${spriteSize/grapheme.length}px sans-serif`;
            }
    ...
    draw(ctx) {
        ...
        this.words.forEach((w, i) => {
            w.forEach(p => {
                ...
                c = c === "silent" || level ? this.colors[c] : "silver";
                ctx.save();
                ctx.translate(px, py);
                ctx.fillStyle = c;
                ctx.fillText(s, 0, 0);
